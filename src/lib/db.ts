...
function mapJob(r: Row): Job {
  return {
    ...
    featuredUntil:       r.featured_until ? new Date(r.featured_until as string) : null,
  };
}
...
function mapCompanyProfile(r: Row): CompanyProfile {
  return {
    ...
    plan:         (r.plan ?? 'free') as 'free' | 'growth' | 'scale',
  };
}
...
export async function patchJob(id: string, updates: Partial<Job>): Promise<void> {
  ...
  if (updates.featured           !== undefined) dbPatch.featured           = updates.featured;
  if (updates.featuredUntil      !== undefined) dbPatch.featured_until      = updates.featuredUntil ? (updates.featuredUntil as Date).toISOString() : null;
